...
Created build-tagged cmd wiring files (run_tui_stub.go for !tui, run_tui.go for tui) and 9 model unit tests in internal/tui/model_test.go. Added TUI mode check in run.go. All tests pass, both build variants compile.
...
```
45f0e81 feat(tui): cmd wiring and model unit tests (FEAT-051)
```
